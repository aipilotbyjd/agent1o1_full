...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('webhooks', function (Blueprint $table) {
            ...
            $table->foreignUuid('workflow_id')->unique()->constrained('workflows')->cascadeOnDelete();
            $table->foreignUuid('workspace_id')->constrained('workspaces')->cascadeOnDelete();
            $table->uuid('uuid')->unique();
            $table->string('path', 100)->nullable();
            $table->json('methods')->default('["POST"]');
            $table->boolean('is_active')->default(true);
            $table->enum('auth_type', ['none', 'header', 'basic', 'bearer']);
            $table->text('auth_config')->nullable();
            $table->unsignedInteger('rate_limit')->nullable();
            $table->enum('response_mode', ['immediate', 'wait']);
