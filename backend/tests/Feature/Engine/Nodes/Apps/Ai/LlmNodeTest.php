...
it('handles structured_extract operation', function () {
    Ai::fakeAgent(\App\Ai\Agents\StructuredExtractAgent::class, [['name' => 'John', 'age' => 30]]);

    $payload = llmPayload(
        config: ['operation' => 'structured_extract', 'schema' => ['name' => ['type' => 'string'], 'age' => ['type' => 'number']]],
        inputData: ['text' => 'John is 30 years old']
    );

    $result = (new LlmNode)->handle($payload);

    expect($result->status)->toBe(ExecutionNodeStatus::Completed)
        ->and($result->output['data']['name'])->toBe('John')
        ->and($result->output['data']['age'])->toBe(30);
});

it('handles vision operation', function () {
    Ai::fakeAgent(\App\Ai\Agents\VisionAgent::class, ['A beautiful landscape']);

    $payload = llmPayload(
        config: ['operation' => 'vision'],
        inputData: ['prompt' => 'Describe this', 'image_url' => 'https://example.com/image.jpg']
    );

    $result = (new LlmNode)->handle($payload);

    expect($result->status)->toBe(ExecutionNodeStatus::Completed)
        ->and($result->output['description'])->toBe('A beautiful landscape');
});

it('handles tts operation', function () {
    \Illuminate\Support\Facades\Storage::fake('local');
    // Laravel Ai fakeAudio doesn't need parameters by default
    ...
    expect($result->status)->toBe(ExecutionNodeStatus::Completed)
        ->and($result->output['audio_url'])->not->toBeEmpty();
});

it('handles transcribe operation', function () {
    \Laravel\Ai\Transcription::fake(['Hello world transcription']);

    $payload = llmPayload(
        config: ['operation' => 'transcribe'],
        inputData: ['audio_url' => 'https://example.com/audio.mp3']
    );

    $result = (new LlmNode)->handle($payload);

    expect($result->status)->toBe(ExecutionNodeStatus::Completed)
        ->and($result->output['text'])->toBe('Hello world transcription');
});
