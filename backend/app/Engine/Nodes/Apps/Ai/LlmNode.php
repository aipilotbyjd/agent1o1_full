...
use App\Agents\Internal\SentimentAgent;
use App\Agents\Internal\StructuredExtractAgent;
use App\Agents\Internal\SummarizerAgent;
use App\Agents\Internal\TextClassifierAgent;
use App\Agents\Internal\VisionAgent;
...
use Laravel\Ai\Files\Audio as AiAudio;
...
class LlmNode extends AppNode
{
    ...
    protected function operations(): array
    {
        return [
            ...
            'structured_extract' => $this->structuredExtract(...),
            'vision' => $this->vision(...),
    ...
    private function structuredExtract(NodePayload $payload): array
    {
        ...
        $agent = new StructuredExtractAgent($schema);
    ...
    private function vision(NodePayload $payload): array
    {
        ...
        $attachments = [];
        if ($imageUrl) {
            $attachments[] = AiImage::fromUrl($imageUrl);
        }
    ...
    private function tts(NodePayload $payload): array
    {
        ...
        $audio = Audio::of($text)->voice($voice);

        $result = $provider
            ? $audio->generate($provider, $model)
            : $audio->generate();

        $storedPath = $result->store('ai-audio', config('filesystems.default'));
    ...
    /**
     * Transcribe speech to text using the SDK.
     *
     * @return array<string, mixed>
     */
    private function transcribe(NodePayload $payload): array
    {
        ...
        $audioFile = str_starts_with($audioUrl, 'http')
            ? AiAudio::fromUrl($audioUrl)
            : AiAudio::fromStorage($audioUrl);

        $transcription = Transcription::of($audioFile);

        $result = $provider
            ? $transcription->generate($provider, $model)
