...
class OutputBuffer
{
     ...
     *
     * Uses a fast recursive estimate instead of a full json_encode to avoid
     * CPU overhead on large output sets.
     */
    public function memoryUsage(): int
    {
        return $this->estimateArrayBytes($this->outputs);
    }

    /**
     * Cheap recursive byte estimator — avoids serialising the whole array.
     *
     * @param  mixed  $value
     */
    private function estimateArrayBytes(mixed $value): int
    {
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $k => $v) {
                $size += strlen((string) $k) + $this->estimateArrayBytes($v) + 16;
            }
