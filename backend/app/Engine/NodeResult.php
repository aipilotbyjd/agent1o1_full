...
class NodeResult
{
    ...
    /**
     * A failed node that should route its error data to the special "error"
     * output handle instead of halting execution.
     *
     * The node's status remains Failed so callers can detect it, but
     * activeBranches is set to ['error'] so the frontier only advances
     * through edges whose sourceHandle is "error".
     *
     * The error data is also surfaced as the node's output so downstream
     * nodes can read it via expression syntax (e.g. {{ nodes.myNode.error }}).
     *
     * @param  array<string, mixed>  $errorData
     */
    public static function errorOutput(array $errorData, int $durationMs = 0): self
    {
        return new self(
            status: ExecutionNodeStatus::Failed,
