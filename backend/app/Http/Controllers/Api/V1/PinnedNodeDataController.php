...
class PinnedNodeDataController extends Controller
{
     ...
     */
    public function toggle(Workspace $workspace, Workflow $workflow, PinnedNodeData $pinnedData): JsonResponse
    {
        $this->can(Permission::WorkflowUpdate);

        $pin = $this->pinnedDataService->toggleActive($pinnedData);
        $pin->load('pinnedBy');
     ...
     */
    public function destroy(Workspace $workspace, Workflow $workflow, PinnedNodeData $pinnedData): JsonResponse
    {
        $this->can(Permission::WorkflowUpdate);

        $this->pinnedDataService->unpin($pinnedData);
